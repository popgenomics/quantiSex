...
void panmixie(gsl_rng* r, Deme* population, Deme* newPopulation, const int nDemes, const int nNtrlLoci, const int nQuantiLoci, const double ntrlMutation, const double quantiMutation, const int fecundity, const double sexAvantage, const int sexualSystem, const double selfingRate, const double dispersal);
...
int main(int argc, char *argv[]){
	...
	const double dispersal = atof(argv[11]);	// pollen dispersal rate 
	...
	const int lapse_stats = atoi(argv[18]); // print statistics every "lapse_stats" generations
	const int seed = atoi(argv[19]);
...
//	printf("%lf %lf %lf\n", global_Fst_anova, global_Fis_anova, global_Fit_anova);
	...
	for(i=0; i<=nGeneration; i++){	// start of the loop 'i' over the generations
		...
		panmixie(r, population, newPopulation, nDemes, nNtrlLoci, nQuantiLoci,  ntrlMutation, quantiMutation, fecundity, sexAvantage, sexualSystem, selfingRate, dispersal);
...
void panmixie(gsl_rng* r, Deme* population, Deme* newPopulation, const int nDemes, const int nNtrlLoci, const int nQuantiLoci, const double ntrlMutation, const double quantiMutation, const int fecundity, const double sexAvantage, const int sexualSystem, const double selfingRate, const double dispersal){
	...
	int cntInd = 0;
		...
		for(i=0; i<nDemes; i++){	// start the loop over the nDemes
			...
			double* fathers_otherDemes = NULL; // array containing the fathers ID in case of pollen dispersal
			...
			// prepare vectors of individuals to deal with pollen dispersal
			// count the number males in the metapop
			int nMalesMetapop = 0;
			for(j=0; j<nDemes; j++){
				if(j!=i){
					nMalesMetapop += population[j].nIndividus;
				}
			}
			
			double* maleID = NULL; // contains ID specific to each male (from 0 to nMalesMpetapop-1)
			double* maleAllocation = NULL; // contains male allocations
			int* demeID = NULL; // deme's IDs of different males
			int* posInDeme = NULL; // position within its own deme of a given male
			
			int nHybridizedFemales = 0; // number of females fertilized by a pollen from a different deme
			int* hybridizedFemales_tmp = NULL; // IDs of females feritlized by a pollen from a different deme
			
			maleID = malloc(nMalesMetapop * sizeof(double));
			maleAllocation = malloc(nMalesMetapop * sizeof(double));
			demeID = malloc(nMalesMetapop * sizeof(int));
			posInDeme = malloc(nMalesMetapop * sizeof(int));
			
			hybridizedFemales_tmp = malloc(N * sizeof(int)); // of size N because it's the maximum expected number of mothers fertilized by different demes
			
			if(maleID == NULL || maleAllocation == NULL || demeID == NULL || posInDeme == NULL || hybridizedFemales_tmp == NULL){
				exit(0);
			}
			...
			for(j=0; j<nDemes; j++){
				if(j!=i){
					for(tmp=0; tmp<population[j].nIndividus; tmp++){
						maleID[cntInd] = cntInd;
						maleAllocation[cntInd] = population[j].maleAllocation[tmp];
						demeID[cntInd] = j;
						posInDeme[cntInd] = tmp;
						cntInd += 1;
					}
				}
			}
			...
			// shorten the list of hybridized females
			int* hybridizedFemales = NULL;
			hybridizedFemales = malloc(nHybridizedFemales * sizeof(int));
			
			if(hybridizedFemales == NULL){
				exit(0);
			}
			
			for(j=0; j<nHybridizedFemales; j++){
				hybridizedFemales[j] = hybridizedFemales_tmp[j];
			}
			
			// if at least one ovule is fertilized by one pollen from a different deme
			if(nHybridizedFemales>0){
				fathers_otherDemes = malloc(nHybridizedFemales * sizeof(double));
				
				if(fathers_otherDemes == NULL){
					exit(0);
				}
				
				// print for debugs
/*				printf("n fertilized females by distant pollen: %d\n", nHybridizedFemales);
				
				printf("Male ID: ");
				for(tmp=0; tmp<nMalesMetapop; tmp++){
					printf("%f\t", maleID[tmp]);
				}
				printf("\n");

				printf("Male allocation: ");
				for(tmp=0; tmp<nMalesMetapop; tmp++){
					printf("%f\t", maleAllocation[tmp]);
				}
				printf("\n");
				...
				printf("Position in deme: ");
				for(tmp=0; tmp<nMalesMetapop; tmp++){
					printf("%d\t", posInDeme[tmp]);
				}
				printf("\n");
...
//				weightedSample(r, maleID, maleAllocation, fathers_otherDemes, K, nHybridizedFemales);
				weightedSample(r, maleID, maleAllocation, fathers_otherDemes, cntInd, nHybridizedFemales);
			}
			...
			int fertilizedByDispersal_cnt = 0;
			
			for(j=0; j<N; j++){	// loop over the babies.  Transmission of alleles, meiosis = gsl_ran_binomial
				int testExternalPollen = 0;
				testExternalPollen = valueInArray(j, nHybridizedFemales, hybridizedFemales);
				...
				for(tmp=0; tmp<nNtrlLoci; tmp++){	// loop along the neutral loci
					...
					if(testExternalPollen==0 || nHybridizedFemales==0 ){
						pos2 = fathers[j]*nNtrlLoci*2 + tmp*2 + gsl_ran_binomial(r, 0.5, 1);
						newPopulation[i].ntrlLoci[pos] = population[i].ntrlLoci[pos2];
					}else{
						tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
						pos2 = posInDeme[tmp2]*nNtrlLoci*2 + tmp*2 + gsl_ran_binomial(r, 0.5, 1);

						tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
						tmp2 = demeID[tmp2];
						newPopulation[i].ntrlLoci[pos] = population[tmp2].ntrlLoci[pos2];
					}
				...
				for(tmp=0; tmp<nQuantiLoci; tmp++){	// loop along the quantitative loci
					...
					if(testExternalPollen==0 || nHybridizedFemales==0){
						...
						tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
						pos2 = posInDeme[tmp2]*nQuantiLoci*2 + tmp*2 + gsl_ran_binomial(r, 0.5, 1);

						tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
						tmp2 = demeID[tmp2];
						newPopulation[i].quantiLoci[pos] = population[tmp2].quantiLoci[pos2];

//						pos2 = posInDeme[fathers_otherDemes[fertilizedByDispersal_cnt]]*nQuantiLoci*2 + tmp*2 + gsl_ran_binomial(r, 0.5, 1);
//						newPopulation[i].quantiLoci[pos] = population[demeID[fathers_otherDemes[fertilizedByDispersal_cnt]]].quantiLoci[pos2];
				...
				for(tmp=0; tmp<1; tmp++){ // transmission of the sex chromosomes
					...
					if(testExternalPollen==0 || nHybridizedFemales==0){
						...
						tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
						pos2 = posInDeme[tmp2]*2 + tmp*2 + gsl_ran_binomial(r, 0.5, 1);

						tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
						tmp2 = demeID[tmp2];
						newPopulation[i].sexChro[pos] = population[tmp2].sexChro[pos2];
						
//						pos2 = posInDeme[fathers_otherDemes[fertilizedByDispersal_cnt]]*2 + tmp*2 + gsl_ran_binomial(r, 0.5, 1); // sex chromosome from the father
//						newPopulation[i].sexChro[pos] = population[demeID[fathers_otherDemes[fertilizedByDispersal_cnt]]].sexChro[pos2];
					}
				}
				
				if(testExternalPollen==1){
//					tmp2 = fathers_otherDemes[fertilizedByDispersal_cnt];
//					printf("Deme: %d\tFemale: %d\tDeme: %d\tMale: %d\n", i, j, demeID[tmp2], posInDeme[tmp2]);
					fertilizedByDispersal_cnt += 1;
				}
			}	// end of loop along the individuals
...
//				if(newPopulation[i].sexChro[2*j] != newPopulation[i].sexChro[2*j+1]){ // if heterogametic individual
			...
			free(demeID);
			free(posInDeme);
			free(hybridizedFemales_tmp);
			free(hybridizedFemales);
			free(fathers_otherDemes);
...
void checkCommandLine(int argc){
	if(argc != 20){
		...
		%s11.%s Rate of pollen dispersal\n\t\
		...
		%s15.%s sexualSystem is equal to 0 if autosomal, equal to 1 if XY and equal to 2 if ZW\n\t\
		%s16.%s Sexual effects of heterogametic sex (if equal to 1.5 in XY system, males have a 50 percent advantage to sire available ovules). Required but neglected if sexualSystem == 0\n\n\t\
		%s17.%s Selfing rate of hermaphrodites, fixed over time (in [0-1])\n\n\t\
		%s18.%s Frequency at which statistics are written in the outfile (>0)\n\n\t\
		%s19.%s Seed for the random generator (>0)\n\n\
		%s\tExample:%s ./quantiSex 100 100 100 50 10 0.0001 1 0.00001 100 1 0.05 0.1 1 1 0 2 0.1 20 123\n\n\
		version: %s\n\n\t\tdependencies: \t%s\n\n", KRED, STOP, KRED, argc-1, STOP, KRED, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KMAG, STOP, KRED, STOP, VERSION, DEPENDENCY);
